...
def check_query(answer, maildomain):
    # wouldn't it be easier to compare the original query
    # and the query in answer ? a gain in code readability ?
    # maybe a better trade-of between memory and processing
    legit = True
...
def extract_answers(answer, answercount):
    ...
    fullanswer = answer
    # keep a full copy for name ptr sake

    answer = answer.split(chr(0)+chr(15), 1)[1][2:]
    # drop the original answer part for easier calculations
    ...
    while k < len(answer):
        ...
        if bin(ord(answer[k]))[2:].zfill(8)[:2] == '11':
            ...
            k += 1+int(answer[k])
            # skip the name, although maybe we should care
        ...
        k += 4  # skip the TTL, we're cachin' nothin' (is it still true ?)
        ...
        while l < len(mxdnsname):
            ...
            if bin(ord(d))[2:].zfill(8)[:2] == '11':
                ...
                mxdnsname = mxdnsname[:l]
                mxdnsname += fullanswer[int(p1+p2, 2):].split(chr(0), 1)[0]
                mxdnsname += chr(0)
                mxdnsname += mxdnsname[l+2:]
...
socket.setdefaulttimeout(conf.SKTTIMEOUT)
# configure a timeout for the socket module
domainmx = {}
# cache the queried domain in case of multiple addresses in the same domain
...
for mailaddr in MAILADDRS:
    ...
    if maildomain not in domainmx:
        ...
        transid = ''.join([chr(int(transid[x*8:(x+1)*8], 2))
                           for x in range(len(transid)/8)])

        sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        # connecting in IP/UDP
        try:
            sock.sendto(msg, (DNSIP, DNSPORT))
            # to the DNS, asking for the mx of the domain
            data, server = sock.recvfrom(2048)
        except Exception as e:
            # connectivity issue, DNSIP issue, DNSPORT issue, probably
            print '\terror: socket to '+DNSIP+' on port '+str(DNSPORT)+' (UDP)'
            print '\terror: '+str(e)
            break
        finally:
            sock.close()
        ...
        if okayheader:
            ...
            if okayquery:
                ...
                if len(answers) > 0:
                    ...
                    domainmx[maildomain] = ['NOPE',
                                            'Empty answer array (DNS is not forwarding probably)']
            else:
                domainmx[maildomain] = ['NOPE',
                                        'Compromised answer (not okayquery)']
        else:
            domainmx[maildomain] = ['NOPE',
                                    'No valid MX answer (not okayheader)']
